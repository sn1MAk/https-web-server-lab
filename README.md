# HTTPS Web Server Lab
## Тема
Розгортання веб-сервера Nginx, налаштування HTTPS із використанням OpenSSL та виведення у публічний доступ до локального веб-сайту через тунель ngrok.


## Мета роботи

- встановити та налаштувати Nginx;
- створити веб-сторінку;
- створити приватний ключ та SSL-сертифікат за допомогою OpenSSL;
- налаштувати HTTPS у Nginx;
- перевірити роботу веб-сервера в локальній мережі;
- забезпечити публічний доступ до локального сервера через ngrok;
- оформити результати лабораторної роботи та розмістити матеріали на GitHub.

## 1.Використані технології та ПЗ

- Windows 11
- WSL2(Ubuntu)
- Nginx
- OpenSSL
- PowerShell
- ngrok
- Git
- GitHub

## 2.Теоритичні відомості

### 2.1Структура проєкту

```text
https-web-server-lab/
├── website/
│   └── index.html
├── nginx/
│   └── https-lab.conf
├── ssl/
│   └── README.md
├── screenshots/
├── .gitignore
└── README.md
```

### 2.2.Вер-сервер Nginx

**Nginx** — веб-сервер та reverse proxy, який може приймати HTTP та HTTPS-запити від клієнтів і повертати необхідні ресурси.

Схематично робота сервера має вигляд:
```
Клієнт
   ↓
HTTP / HTTPS
   ↓
Nginx
   ↓
/var/www/html/index.html
```

### 2.3.HTTP та HTTPS

**HTTP** використовується для передачі даних між клієнтом і веб-сервером.

**HTTPS** є захищеною версією HTTP та використовує протоколи TLS для забезпечення:
- конфіденційності
- цілісності даних
- автентифікації сервер
Під час використання **HTTPS** дані між клієнтом та сервером передаються через зашифроване **TLS**-з'єднання.

### 2.4.~~SSL~~/TSL-сертифікат

> [!note] 
> **SSL** - застарілий протокол розроблений у 1990-х роках, на зміну якому прийшов швидший та надійніший TSL у 1999-му році. Але у народі так і залишися назва SSL.

У роботі були створені:
```
/etc/nginx/ssl/server.key
/etc/nginx/ssl/server.crt
```
> [!warning]
> `server.key` — приватний ключ сервера. Він повинен зберігатися в секреті, тому у репозиторій не буде доданий.

`server.crt` — X.509-сертифікат, який містить відкритий ключ та інформацію про суб'єкта сертифіката, термін його дії та криптографічний підпис.

У роботі використовується *самопідписаний сертифікат(self-signed)*, тому браузер не довіряє та викидає червону табличку.

## 3.Розгортання веб-сервера

### 3.1.Встановлення Nginx
У Ubuntu WSL2 виконав оновлення списку пакетів та встановлення Nginx:
```bash
sudo apt update & sudo apt install nginx -y
```

Після встановлення перевіримо стан веб-сервера командою:
```bash
sudo systemctl status nginx
```
![sudo systemctl status nginx](https://github.com/sn1MAk/https-web-server-lab/blob/fa597aa3227ba3c41f43c4ed90970b0aef25c71c/screenshots/01-nginx-status.png)

Після встановлення **Nginx** став активним.

За допомогою команди:
```bash
sudo ss -tulpn | grep -E '80'
```
![nginx listening port](https://github.com/sn1MAk/https-web-server-lab/blob/9ad313e14969d266e41244aa3689c4ff0d5a2827/screenshots/02-nginx-listening-port.png)

Перевірили, що **Nginx** прослуховує порт `80`.


### 3.2.Створення веб-сторінки

Файли сайту розміщуються в стандартній теці Nginx:
```bash
/var/www/html/
```

Основним файлом сайту є:
```bash 
/var/www/html/index.html
```
Саме цей файл повертається Nginx-ом на запит клієнта.

Після налаштування HTTP роботу сервера перевірено через 
```bash 
http://localhost
```
У результаті веб-сторінка успішно відкрилася та успішно працювала через HTTP протокол та `80` порті *(стандартному для http)*.

![web site](https://github.com/sn1MAk/https-web-server-lab/blob/6926d93315307b962fef155586572f6d8e436d21/screenshots/03-web-site.png)

# 4.Налаштування HTTPS

## 4.1.Створення директорії для TSL-файлів

Для зберігання ключа та сертифіката була створена директорія:
```bash
sudo mkdir -p /etc/nginx/ssl
```

## 4.2.Створення приватного ключа

За допомогою OpenSSL було створено RSA-ключ довжиною 2048 біт.
```bash
sudo openssl genrsa -out /etc/nginx/ssl/server.key 2048
```
Командою створено ключ-файл `server.key`.

## 4.3.Створення самопідписаного сертифіката

Було створено X.509-сертифікат прив`язаний до приватного ключа:
```bash
sudo openssl req -new -x509 \
-key /etc/nginx/ssl/server.key \
-out /etc/nginx/ssl/server.crt \
-days 30
```

Під час створення сертифіката було вказано:
```bash
Common Name: localhost
```

Термін дії сертифіката - 30 днів.

# 5.Перевірка TSL-сертифіката

Для перегляду сертифіката використана команда:
```bash 
openssl x509 -in /etc/nginx/ssl/server.crt -text
```
Команда дозволяє переглянути:
- версію X.509;
- серійний номер;
- алгоритм підпису;
- видавця;
- термін дії;
- Subject;
- відкритий ключ;
- розширення сертифіката;
- цифровий підпис.
![tsl crt](https://github.com/sn1MAk/https-web-server-lab/blob/9d99c1d1d8d742fbcf310185f8efa6b98b6746c8/screenshots/04-tsl-crt.png)

## 5.1.Перевірка TSL-з`єднання

Для перевірки TLS-з'єднання між клієнтом та Nginx було використано:

```bash
openssl s_client -connect localhost:443
```
Команда `s_client` виступає у ролі TLS-клієнта та встановлює TLS-з'єднання з сервером на порту `443`.

# 6.Налаштування HTTPS у Nginx
Було створено конфігураційний файл:
```bash
/etc/nginx/sites-available/https-lab
```

Конфіг знаходиться [за посиланням](https://github.com/sn1MAk/https-web-server-lab/blob/283d3f1c94972089f9a2296742776167b2249895/nginx/https-lab.conf) або `nginx/https-lab.conf`.

## 6.1.Підключення конфігурації
Конфігурація була підключена символічним посиланням:
```bash
sudo ln -s /etc/nginx/sites-available/https-lab /etc/nginx/sites-enabled/
```
Після цього конфігурацію було перевірено:
```bash
sudo nginx -t
```
Перевірка була успішна.

Після цього конфігурацію Nginx було перезавантажено:
```bash
sudo systemctl reload nginx
```
> [!important]
> При чому було використано `reload`, а не `restart`.
> `restart` вимикає та запускає програму.
> `reload` переглядає всі конфіги та перезапускається з новими налаштуваннями.

# 7.Перевірка HTTPS
Після налаштування HTTPS веб-сайт став доступним за посиланням:
```bash
https://localhost
```
Але, як і було очікувано, браузер не довіряє цьому сертифікатові. Тому вилазить червона табличка та ~~https~~.

![https site](https://github.com/sn1MAk/https-web-server-lab/blob/d122f9004da584ce1076490e85470661eb36945c/screenshots/05-https-site.png)

# 8.Перевірка мережевої доступності
Для перевірки портів Nginx було використано:
```bash
sudo ss -tulpn | grep -E ':80|:443'
```
З`ясовано, що Nginx слухає:
```bash
0.0.0.0:80
0.0.0.0:443
```
а також відповідні IPv6 адреси.

# 9.Особливості мережі WSL2
Оскільки Nginx працює в середині WSL2, Linux має власне ip, яке було встановлене за допомогою:
```powershell
ip addr
```
Після цього перевірено доступ до Nginx з Windows через ip-адрес WSL2:
```powershell
curl http://<WSL_IP>
```
Команда повернула код сторінки. Це означає, що Nginx та WSL працюють правильно.

# 10.Налаштування доступу через Windows у локальній мережі 
Для перенаправлення трафіку з Windows до WSL був використаний механізм `portproxy`.

Команда для налаштування перенаправлення на HTTP:
```powershell
netsh interface portproxy add v4tov4 listenaddress=0.0.0.0 listenport=80 connectaddress=<WSL_IP> connectport=80
```
Для HTTPS:
```powershell
netsh interface portproxy add v4tov4 listenaddress=0.0.0.0 listenport=443 connectaddress=<WSL_IP> connectport=443
```

Після цього система доступу у локальній мережі стала такою:
```text
Клієнт
   ↓
Windows IP
   ↓
Windows portproxy
   ↓
WSL IP
   ↓
Nginx
```

Наявність перенаправлення була перевірена:
```powershell
netsh interface portproxy show all
```

![netsh port](https://github.com/sn1MAk/https-web-server-lab/blob/fdfb9e3e96d378869087b002844af9794523c098/screenshots/06-netsh-port.png)

# 11.Налаштування Windows Firewall

Для дозволу входу HTTP було створено правило:
```powershell
New-NetFirewallRule `
-DisplayName "WSL Nginx HTTP Lab" `
-Direction Inbound `
-Protocol TCP `
-LocalPort 80 `
-Action Allow
```
Для HTTPS:
```powershell
New-NetFirewallRule `
-DisplayName "WSL Nginx HTTPS Lab" `
-Direction Inbound `
-Protocol TCP `
-LocalPort 443 `
-Action Allow
```
 
Після цього веб-сайт став доступним з інших пристроїв підлючених до тієї самої мережі Wi-Fi/vpn:

![mobile test](https://github.com/sn1MAk/https-web-server-lab/blob/d4c1c408c678c15e9b060ec8bd73151daf341482/screenshots/07-mobile-test.png)

# 12.Забезпечення публічного доступу до Інтернету

Через те, що сервер працює фізично на ноутбуці, для доступу в інтернет використаний ngrok

ngrok створює захищений тунель між локальним сервером та інтернет.

Загальна схема:
```text
                     INTERNET
                         │
                         ▼
               HTTPS / ngrok URL
                         │
                         ▼
                      ngrok
                         │
                         ▼
                  Windows laptop
                         │
                         ▼
                       WSL2
                         │
                         ▼
                      Nginx
                         │
                         ▼
                    index.html
```

Ngrok був налаштований по інструкції з [офіційного сайту ngrok.com](https://ngrok.com/)

Публічна адреса видана Ngrok-ом:
```
https://resort-champion-enclosure.ngrok-free.dev
```

Запущений `ngrok-тунель`:
![ngrok console](https://github.com/sn1MAk/https-web-server-lab/blob/ead6a9af8663b5d1f65630969897affcd9c3056d/screenshots/08-ngrok-console.png)

Щоб зупинити хостинг потрібно закрити консоль, або в консолі нажати сполучення клавіш <kbd>CTRL</kbd> + <kbd>C</kbd>

Сайт через публічний доступ:
![ngrok site](https://github.com/sn1MAk/https-web-server-lab/blob/ead6a9af8663b5d1f65630969897affcd9c3056d/screenshots/09-ngrok-site.png)

## 13.1.Особливість HTTPS при використанні ngrok
У локальній частині лабораторної було налаштовано власний HTTPS-сервер Nginx із використанням OpenSSL.

При публічному доступі через ngrok TLS-з'єднання для користувача завершується на стороні ngrok. Тому користувач отримує публічний HTTPS-доступ без попередження про мій локальний самопідписаний сертифікат.

Таким чином я продемонстрував **два рівні HTTPS**:
```text
Локальна лабораторна частина:

Browser
   ↓ HTTPS
Nginx
   ↓
Self-signed certificate
```
та:
```text
Публічний доступ:

Browser
   ↓ HTTPS
ngrok
   ↓
локальний сервер
   ↓
Nginx
```

Це дозволило одночасно продемонструвати налаштування TLS у Nginx та зробити сайт доступним через Internet без оренди VPS або придбання домену.

## 14.Результат роботи
У результаті виконання лабораторної роботи було:

- Встановлено веб-сервер Nginx у WSL2 Ubuntu.
- Створено та розгорнуто HTML-веб-сайт.
- Перевірено роботу веб-сайту через HTTP.
- Створено RSA-приватний ключ за допомогою OpenSSL.
- Створено X.509 самопідписаний сертифікат.
- Налаштовано HTTPS у Nginx на порту 443.
- Перевірено конфігурацію Nginx.
- Перевірено TLS-з'єднання за допомогою openssl s_client.
- Перевірено структуру сертифіката за допомогою openssl x509.
- Налаштовано перенаправлення портів Windows → WSL.
- Налаштовано правила Windows Firewall.
- Перевірено доступ до сайту з інших пристроїв.
- Забезпечено публічний доступ до локального веб-сервера через Internet за допомогою ngrok.
- Підготовлено матеріали лабораторної роботи для розміщення на GitHub.
